...
export function safeshotConfig(env) {
  if (!env || typeof env !== "object") {
    throw new Error("safeshotConfig requires env");
  }
  const preorderUrl =
    typeof env.SAFESHOT_PREORDER_URL === "string" ? env.SAFESHOT_PREORDER_URL.trim() : "";
  return {
    preorderUrl: preorderUrl.length > 0 ? preorderUrl : null,
  };
}
